...
import "./selectFetch.css"

export default (({ setFetch, setUrl, setLoading }) => {
    ...
    return (
        <div className="select-fetch">
            <button className="select-fetch__button button_color_green" onClick={() => onSelectUrl(smallUrl)}>{`32 элемента`}</button>
            <button className="select-fetch__button button_color_red" onClick={() => onSelectUrl(bigUrl)}>{`1000 элементов`}</button>
        </div>
    )
}
